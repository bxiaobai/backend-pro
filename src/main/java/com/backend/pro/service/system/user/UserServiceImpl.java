...
import com.backend.pro.mapper.UserMapper;
...
import com.backend.pro.model.enums.UserRoleEnum;
...
import static com.backend.pro.constant.UserConstant.USER_LOGIN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public LoginUserVO userLoginByMpOpen(WxOAuth2UserInfo wxOAuth2UserInfo, HttpServletRequest request) {
        ...
        synchronized (unionId.intern()) {
            ...
            if (user != null && UserRoleEnum.BAN.getValue().equals(user.getUserRole())) {
                throw new BusinessException(FORBIDDEN_ERROR, "该用户已被封，禁止登录");
            }
            // 用户不存在则创建
            if (user == null) {
                user = new User();
                ...
                if (!result) {
                    throw new BusinessException(SYSTEM_ERROR, "登录失败");
                }
            }
            // 记录用户的登录态
            request.getSession().setAttribute(USER_LOGIN_STATE, user);
    ...
    public LoginUserVO getLoginUserVO(User user) {
        ...
        loginUserVO.setDeptName(dept.getDeptName());
        //放入数据
    ...
    public static void main(String[] args) {
        String encryptPassword = DigestUtils.md5DigestAsHex((SALT + "admin").getBytes());
